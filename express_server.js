...
app.get("/urls", (req, res) => {
  ...
  if (!userID) {
    return res
      .status(401)
      .send("401: Please go back and log in; or register first.");
  }
...
app.get("/urls/:shortURL", (req, res) => {
  ...
  if (!urlDatabase.hasOwnProperty(shortURL)) {
    return res.status(404).send("404: The page requested was not found.");
  }
  const userID = req.session.user_id;
  const user = userDatabase[userID];
  const templateVars = {
...
app.post("/register", (req, res) => {
  ...
  if (!email || !password) {
    return res
      .status(400)
      .send("Please enter an email address and password to register.");
  }
...
app.post("/login", (req, res) => {
  ...
  if (!email || !password) {
    return res
      .status(400)
      .send("400: Please enter an email address and password to login.");
  }
  // Check if email already exists in the database:
